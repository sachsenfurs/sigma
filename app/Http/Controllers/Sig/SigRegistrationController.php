...
class SigRegistrationController extends Controller
{
    ...
    public function register(SigTimeslot $timeslot) {
        ...
        } elseif($timeslot->timetableEntry->sigEvent->group_registration_enabled && $timeslot->sigAttendees->count() > 0 &! $timeslot->sigAttendees->where('user_id', auth()->user()->id)->where('timeslot_owner', true)->first()) {
