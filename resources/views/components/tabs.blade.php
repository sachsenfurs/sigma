        ...
        @foreach($tabs AS $index => $tab)
            @isset($__laravel_slots[$tab])
                <li class="nav-item flex-fill" role="presentation">
                    <button @class(["nav-link w-100 h-100", "active" => $loop->index == 0]) id="{{ \Illuminate\Support\Str::kebab($tab) }}-tab"
                            data-bs-toggle="tab" data-bs-target="#{{ \Illuminate\Support\Str::kebab($tab) }}-tab-pane" type="button" role="tab"
                            aria-controls="{{ \Illuminate\Support\Str::kebab($tab) }}-tab-pane" aria-selected="{{ $loop->index == 1 ? "true":"false" }}">
                        <h4>
                            {{ __($tab) }}
...
<div class="tab-content" id="tabContent">
    @foreach($tabs AS $tab)
        <div @class(["tab-pane fade", "show active" => $loop->index == 0]) id="{{ \Illuminate\Support\Str::kebab($tab) }}-tab-pane"
