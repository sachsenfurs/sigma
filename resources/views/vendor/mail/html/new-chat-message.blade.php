...
@foreach($messages AS $message)
        ...
        @if($previous = $message->chat->messages()->to($message->user)->where("read_at", "<", $message->created_at)->latest()->first())
            ...
            <div style="display: flex; align-items: center; margin: 10px 0;">
                ...
                @if($previous->user->avatar_thumb)
                    <img
                        style="width: 40px; height: 40px; border-radius: 50%; margin-right: 0; margin-left: 10px; align-self: end;"
                        src="{{ $previous->user->avatar_thumb }}"
                        alt="">
                @endif
            </div>
        @endif
    ...
    <div style="display: flex; align-items: center; margin: 10px 0;">
        @if($message->user->avatar_thumb)
            <img
                style="width: 40px; height: 40px; border-radius: 50%; margin-right: 10px; align-self: end;"
                src="{{ $message->user->avatar_thumb }}"
                alt="">
        @endif
